...
ExtValidation <- function(wd,test,mtd,times,fmfile,CI,fpr.fixed){
  ...
  for(i in 1:nrow(Xformula)){
    ...
    if(mtd[1] == "bootstrap"){
      ...
      ndat <- round(dim(dat_test)[1]/as.numeric(mtd[2]),0)
    }
    
    mod2 <- update(mod1, setCoeffs(fm1, givenWei, ndat),family = binomial(link='logit'))
    ...
    for(n in 1:nSam){
      ...
      if(nlevels(as.factor(testdat[,X1]))==2){
        ...
        p <- predict(mod2,testdat,type="response")
...
#ExtValidation(wd,test,mtd,times,fmfile,CI,fpr.fixed)
